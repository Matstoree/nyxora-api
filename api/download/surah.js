...
module.exports = function (app) {
  ...
  const daftarSurah = [
    ...
    "57. Al-Hadid","58. Al-Mujadila","59. Al-Hashr","60. Al-Mumtahanah","61. As-Saff",
  ...
  async function getAudioList() {
    ...
    $("a[href$='.mp3']").each((_, el) => {
      ...
      if (!href) return
      const url = new URL(href, SCRAPE_URL).href
      result.push({ url })
    })
  ...
  app.get("/download/surah", async (req, res) => {
    try {
      ...
      if (isNaN(n) || n < 1 || n > 114) {
        return res.json({
          status: false,
          error: "Nomor surah tidak valid"
        })
      }

      const audioList = await getAudioList()
      const kode = String(n).padStart(3, "0")
      const audio = audioList.find(v => v.url.includes(`/${kode}.mp3`))
