...
const NEOXR_KEY = 'NyxoraApikey'

async function getTurnstileToken() {
  const { data: site } = await axios.get(
    'https://api.neoxr.eu/api/get-sitekey',
    {
      params: {
  ...
  if (!site.status) throw new Error('Gagal ambil sitekey')

  const sitekey = site.data.sitekey

  const { data: bypass } = await axios.get(
  ...
  if (!bypass.status) throw new Error('Gagal bypass turnstile')

  return bypass.data.token
}
